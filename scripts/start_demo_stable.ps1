...
$envPairs = @(
  ...
  "JEPA_EVENT_CONTACT_THRESHOLD=0.50",
  "JEPA_EVENT_RELEASE_THRESHOLD=0.20",
  "JEPA_EVENT_RELEASE_UI_THRESHOLD=0.20",
  ...
  "DEMO_MAX_OBJECT_DINO_EMBEDS_PER_UPDATE=2",
...
if ($Sophie) {
  ...
  $envPairs = $envPairs | ForEach-Object {
    ...
    elseif ($_ -like "PERCEPTION_SEGMENTATION_BACKEND=*") { "PERCEPTION_SEGMENTATION_BACKEND=fastsam-s" }
...
if ($DenseObjects) {
  $envPairs = $envPairs | ForEach-Object {
    if ($_ -like "PERCEPTION_SEGMENTATION_BACKEND=*") { "PERCEPTION_SEGMENTATION_BACKEND=$(if ($Sophie) { 'fastsam-s' } else { 'yolo-seg' })" }
